...
export default function ApplyPage({ params }: { params: Promise<{ projectId: string }> }) {
    ...
    useEffect(() => {
        const checkEligibilityAndProject = async () => {
            ...
            try {
                ...
                // Pre-fill email/name if available in auth (optional enhancement)
                if (auth.currentUser.displayName) {
                    setFormData(prev => ({ ...prev, applicantName: auth.currentUser?.displayName || '' }));
                }
        ...
        // Auth listener wrapper not strictly needed if we assume protected route, but good practice
        // We rely on parent layout/context for general auth, but here we need UID specific checks.
        // Let's verify auth exists
        const checkAuth = setTimeout(() => {
            if (auth.currentUser) {
                checkEligibilityAndProject();
            } else {
                // Wait a bit for auth to init, handled by Layout usually, but safe check
            }
        }, 500);
